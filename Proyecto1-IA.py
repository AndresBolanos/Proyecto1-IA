...
def Cruzar_Generacion2(Lista_W, Lista_Loss, Lista_Indices, X, Index):
    ...
    if len(Lista_Indices)%2 != 0:
        W_Loss_Indices.append(Lista_W[Lista_Indices[0]])
        Lista_Indices = Lista_Indices[1:]                             #Quita el más óptimo para no cruzarlo
    #Solo va a recorrer la mitad menos optima y compararlos con la otra mitad más óptima
    for i in range(len(Lista_Indices)/2):
        nuevoW1 = []
        nuevoW2 = []
        PosW_NoOptimo = len(Lista_Indices)-1-i                          #En lista de len=10 se le suma la mitad y sería: 0 con 5, 1 con 6, 2 con 7...
        PosW_Optimo = len(Lista_Indices)-1 -i-(len(Lista_Indices)/2) 
        W_Optimo = Lista_W[Lista_Indices[PosW_Optimo]]                          #Entre más bajo el i, más óptimo el W
        W_NoOptimo = Lista_W[Lista_Indices[PosW_NoOptimo]]
...
def Cruzar_Generacion3(Lista_W, Lista_Loss, Lista_Indices, X, Index):
    ...
    if len(Lista_Indices)%2 != 0:
        ...
        Lista_Indices = Lista_Indices[1:]                             #Quita el más óptimo para no cruzarlo
    #Solo va a recorrer la mitad menos optima y compararlos con la otra mitad más óptima
    contador = 0  #Para saber la posición de los padres
    for i in range(len(Lista_Indices)/2):
        nuevoW1 = []
        nuevoW2 = []
        PosW_NoOptimo = i+contador+1                                         #En lista de len=10 se le suma la mitad y sería: 0 con 5, 1 con 6, 2 con 7...
...
def Compare_Iris_Data(k,mutacion_1,mutacion_2,cruce):
    ...
    for i in range(N):
        ...
        Lista_W = Mutacion_2(Lista_W,Lista_Indices,Lista_Loss,mutacion_1, mutacion_2)  #Mutación de la generación
